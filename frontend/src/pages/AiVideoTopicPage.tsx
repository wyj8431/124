...
import { InspirationVideoCard } from '@/components/ai-topic/InspirationVideoCard'
import {
  InspirationPlaybackProvider,
  useInspirationPlayback,
} from '@/components/ai-topic/InspirationPlaybackContext'
import { AiTopicVideoInput } from '@/components/ai-topic/AiTopicVideoInput'
...
function InspirationSection({
  ...
  const { activeSectionId, reportSectionVisibility } = useInspirationPlayback()
  const canAutoPlay = activeSectionId === sectionId
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        reportSectionVisibility(sectionId, entry.isIntersecting ? entry.intersectionRatio : 0)
      },
    ...
    return () => {
      observer.disconnect()
      reportSectionVisibility(sectionId, 0)
    }
  }, [sectionId, reportSectionVisibility])
...
export function AiVideoTopicPage() {
  ...
  const detailId = useMemo(() => {
    if (searchParams.get('agentDetail') !== '1') return null
          ...
          <section className="ai-topic-input-section">
            ...
            <div className="ai-topic-input-section__canvas">
              <div className="ai-topic-composer">
                ...
                <aside aria-label="预置提示词" className="preset-prompt-stack">
                  {data.presets.map((preset, index) => (
                    <button
                      ...
                      style={{
                        ['--card-x' as string]: `${[99, 209, 303][index] ?? preset.cardOffsetX}px`,
